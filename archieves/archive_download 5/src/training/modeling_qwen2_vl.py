...
from configuration_qwen2_vl_moe import Qwen2VLConfig, Qwen2VLVisionConfig
...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        down_proj, router_logits = self.moe_adapter(
            down_proj, down_proj, router_hidden_sates
        )
...
class Qwen2VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa", use_moe: bool = False) -> None:
        super().__init__()
        self.use_moe = use_moe
    ...
    def forward(
        ...
        mlp_hidden_states, gate_logits = self.mlp(normed_hidden)
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        for blk in self.blocks:
            if self.gradient_checkpointing and self.training:
                hidden_states, gate_logits = self._gradient_checkpointing_func(
                    blk.__call__, hidden_states, cu_seqlens, None, position_embeddings
                )
            else:
                hidden_states, gate_logits = blk(hidden_states, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings)
            if gate_logits is not None:
                gate_logits_list.append(gate_logits)
        if len(gate_logits_list) > 0:
            stacked_logits = torch.cat(gate_logits_list, dim=0)  # [sum(batch * seq), num_experts]
            lb_loss = load_balancing_loss_func(stacked_logits, num_experts=self.config.num_experts)
        else:
            lb_loss = None
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        Args:
            ...
            target_length (`int`):
                The target length: when generating with static cache, the mask should be as long as the static cache, to account for the 0padding, the part of the cache that is not filled yet.
            dtype (`torch.dtype`):
