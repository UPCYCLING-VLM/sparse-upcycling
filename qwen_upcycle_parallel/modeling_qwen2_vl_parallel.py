...
from qwen_upcycle.configuration_qwen2_vl_moe import Qwen2VLConfig, Qwen2VLVisionConfig
...
# A Parallel adapter MLP for MOE
...
class MOEGateAdapter(nn.Module):
...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        moe_down_proj, router_logits = self.moe_adapter(
            router_hidden_states, router_hidden_states, router_hidden_states
        )
        down_proj+=moe_down_proj
        # down_proj = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
        return down_proj, router_logits

#### Original Layers and Blocks
...
class Qwen2VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2VLConfig, layer_idx: int):
        ...
        ## MOE Adapter parallel to attn
        self.attn_moe_adapter = MOEGateAdapter(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_moe_down_proj, attn_moe_router_logits = self.attn_moe_adapter(
        ...
        print("Before moe: ", hidden_states)
        hidden_states+= self.input_layernorm(attn_moe_down_proj)
        print("After moe: ", hidden_states)

        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states, mlp_moe_router_logits = self.mlp(hidden_states)
        hidden_states = residual + hidden_states
        ...
        if output_router_logits:
            outputs += (attn_moe_router_logits, mlp_moe_router_logits, )
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        all_mlp_router_logits = () if output_router_logits else None
        ...
        for decoder_layer in self.layers:
            ...
            if output_router_logits:
                all_attn_router_logits += (layer_outputs[-2],)
                all_mlp_router_logits += (layer_outputs[-1],)
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_attn_router_logits, all_mlp_router_logits,] if v is not None)
        return MoEModelOutputWithPast(
            last_hidden_state=hidden_states,
            past_key_values=next_cache,
            hidden_states=all_hidden_states,
            attentions=all_self_attns,
            router_logits=all_router_logits, ## ATTN AND MLP MOE ROUTERS?
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        attn_aux_loss = mlp_aux_loss = None
        if output_router_logits:
            ...
            if labels is not None:
                loss += 0.01 * attn_aux_loss + 0.01 * mlp_aux_loss
        ...
        return MoECausalLMOutputWithPast(
            loss=loss,
            aux_loss=aux_loss, # ATTN AND MLP MOE OUTPUT?
            logits=logits,
